...
```typescript
// 1. src/commands.ts
export const commands = [{ name: "help", description: "Show help" }] as const;
...
const mentionWallet = await getSmartAccountFromUserId(bot, {
...
(bot.viem, bot.appAddress, bot.botId);

const result = await readContract(bot.viem, {
  address,
  abi,
